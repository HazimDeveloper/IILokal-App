...
<body class="home">
        ...
        <section class="hero bg-image" data-image-src="images/8922.png">
        ...
        <section class="popular">
            <div class="container">
                <div class="title text-xs-center m-b-30">
                    <h2>Laptop Maintenance and Issue Tracking System (LMITS)</h2>
    ...
    <div class="container">
        ...
        <form method="POST" action="">
            <input type="text" name="search_query" placeholder="Enter Laptop No./ Desc ">
		...
		<br>
			...
			$sql = "SELECT * FROM `laptop`  
				WHERE serial_id  ='$data_search'";
			$query_res = mysqli_query($db,"SELECT * FROM laptop WHERE serial_no like '%$data_search%' or problem like '%$data_search%' or current_warrant_date like '%$data_search%' or status like '%$data_search%' "); 
			header('location: index.php');
			
			
}						else { 
						$query_res= mysqli_query($db,"SELECT * FROM `laptop` ORDER BY `serial_no` DESC LIMIT 9 "); 
					}
					while($r=mysqli_fetch_array($query_res))
                                {
                                    ...
                                    <div class="col-xs-12 col-sm-6 col-md-4 food-item">
                                            <div class="food-item-wrap">
                                                <div class="figure-wrap bg-image" data-image-src="admin/Res_img/<?=$r['image']?>"></div>
                                                <div class="content">
                                                    <?php 
                                                    $ress= mysqli_query($db,"SELECT * FROM `restaurant` WHERE title = '".$r['problem']."'"); 
                                                    $result_rss = mysqli_fetch_assoc($ress);
                                                    ?>
                                                    <h5><a href="dishes.php?res_id=<?= $result_rss['rs_id'] ?>"><?= $r['serial_no']?></a></h5>
                                                    <div class="product-name"><?= $r['laptop_desc']?></div>
													
													  <b><br>Date Reported: <div class="product-name"><?= $r['current_warrant_date']?>
                                                    </div></b>
                                                    <?php  $warranty_expiration_date = strtotime($r['expired_warranty_date']);
    ...
    $current_date = date('y-m-d');
                                                    ...
                                                    <b class="style">Warranty Status : <span style="color:red">Expired</span></b><br>

                                                    <?php }else{
                                                    ?>
                                                    <b style="margin-bottom: 10px;" >Warranty Expired: <?php 
                                                                                                 
// Set the current date
$currentDate = new DateTime();

// Set the expired date
$expiredDate = new DateTime($r['expired_warranty_date']);
...
echo $days;
                                                    ?> Day(s) Left</b><br>
                                                    <?php } ?>
                                                    <?php if($r['status']  == 'Good Condition'){  ?>
                                                    <div class="btn btn-success">Good Condition</div>
                                                    <?php }else if($r['status'] == 'Waiting To Repair'){ ?>
                                                        <div class="btn btn-warning">Waiting For Repair</div>
    
                                                        <?php }else if($r['status'] == 'Current Repair'){ ?>                                                    
                                                            <div class="btn" style="background-color: orange;">Currently Repair</div>
              
<?php } else{ ?>
    <div class="btn btn-danger">Broken</div>
